...
3. 끝나면 main에 머지하고 워크트리를 제거한다. 버릴 시도였다면 그냥 제거

### 머지와 충돌 (브랜치·워크트리 공통)

1. `git switch main && git pull`로 main을 최신으로 만든다
2. 충돌이 없으면 그대로 머지한다
3. **충돌이 나면 rebase-merge가 기본이다** — `git switch <branch> && git rebase main`으로
   브랜치를 main 위로 옮겨 해소한 뒤 main에서 머지한다(이 시점엔 fast-forward). 이력이 한
   줄로 남아 "언제 무엇이 배포됐나"를 되짚기 쉽다
4. 아래 예외에 걸리면 **rebase를 진행하지 말고 멈춰서 알린다**

**예외 — 멈추고 노티할 것**

- **브랜치가 이미 origin에 push돼 있다.** rebase는 이력을 다시 쓰므로 force push가 필요해지고,
  다른 세션이 그 커밋을 참조하고 있을 수 있다
...
- **커밋마다 충돌이 반복된다.** 브랜치의 전제가 "중간 커밋은 깨져 있어도 된다"라, 긴 브랜치는
  rebase가 커밋 수만큼 해소를 요구한다. 3회 넘게 반복되면 멈추고 머지 커밋 방식을 상의한다

노티는 **무엇이 충돌했는지 · 왜 예외로 판단했는지 · 선택지**(rebase 강행 / 머지 커밋 /
브랜치 재작성)를 함께 낸다. 임의로 고르지 않는다.

`git rebase -i`는 이 환경에서 지원되지 않는다. 충돌 해소용 `git rebase main`은 문제없지만
squash·reword가 필요하면 사용자에게 요청한다.
