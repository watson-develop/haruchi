...
- `docs/PRD.md` — **살아있는 정책서.** "지금 이 서비스가 무엇이고 어떻게 운영되는가"만
  답한다(스펙은 왜, HANDOFF는 언제). 값을 베끼지 않고 소유자 파일을 가리킨다 — 이 문서와
  코드가 다르면 코드가 맞고 문서를 고친다. **사용자에게 보이는 정책(범위·규칙·역할·화면
  소속)이 바뀌는 커밋마다 해당 절을 덮어쓴다** — HANDOFF 갱신과 같은 커밋에서. 내부
  리팩터·버그픽스는 갱신하지 않는다
...
- `docs/screen-preview.md` — **화면을 기록 오염 없이 보는 법.** 배포본에서 스프린트를 돌리면
  딸의 기록에 섞인다(2026-09-03 실제 발생). 개발 서버 origin에 원하는 정복 칸 수를 심는
  콘솔 스니펫이 여기 있다
- `docs/reference/karrot-DESIGN.md` — 당근(Karrot) 디자인 가이드(oh-my-design.kr 발췌).
  UI 문구·상태·모션 작업 전에 읽는다 — 보이스는 `-어요` 체, 금지 문구 목록, 스프링
  이징 금지, 상태(빈·로딩·에러) 처방이 여기 있다. **문서 안의 hex 값은 참고용이다**
