 ...
 * 호출부(renderManage의 #export 핸들러)가 정책까지 함께 쥔다.
 *
...
// 내보내기·되돌리기 후 배지 갱신을 위해 renderManage가 자기 자신을 다시 부른다(같은
// #/manage 안에서 재렌더 — 해시가 안 바뀐다). importBusy/resetBusy가 함수 스코프
// 지역 변수였다면 그 재렌더마다 새 스코프가 false로 열려 "가져오기 진행 중에 재렌더
// → 새 스코프의 importBusy=false → 가져오기 버튼이 다시 눌려 복구가 두 번 돈다"
// 사고가 났다(최종 브랜치 리뷰에서 발견). 모듈 스코프로 옮겨 여러 renderManage 호출에
// 걸쳐 하나의 진실을 유지한다. 페이지를 새로고침하면(뒤로가기 없이 앱을 새로
// 로드하면) 모듈이 다시 초기화돼 false로 돌아가므로, 새로고침 자체가 영구 잠금의
// 탈출구이기도 하다.
//
// **이 화면은 PIN 게이트 대상이라 재렌더가 리포트 때보다 잦다** — wake 재게이트가
// route(false)로 다시 그린다(main.ts). 모듈 스코프가 그만큼 더 필요하다.
//
 ...
 *
 * 화면끼리 import하지 않는다(형제 규칙) — 리포트와 공유할 것이 생기면 ui.ts로 올린다.
 * 그래서 dayCount·snapshotNotice·dateRange 같은 도우미가 report.ts에서 사라지고 여기
 * 복제가 아니라 **이동**해 왔다.
 *
 * 이 화면의 navigate 목적지는 넷이고 전부 부모 소속이다(CLAUDE.md 아이/부모 불변식):
 * 뒤로가기 `#/report` + 파괴적 흐름 셋(가져오기·초기화·되돌리기)의 `#/parent`.
 */
export async function renderManage(root: HTMLElement): Promise<void> {
  try {
    ...
    const enabledNow = await syncEnabled()
    ...
    root.replaceChildren(
      el(`
        <div>
          ...
          ${enabledNow ? '<h2>연결된 기기</h2><div id="devices-zone">불러오는 중…</div>' : ''}
          <button class="step" id="back">← 리포트</button>
        </div>
      `),
    )

    root.querySelector('#back')!.addEventListener('click', () => navigate('#/report'))
    ...
    const revertExport = (prev: string | null, at: string): void => {
      void getMeta()
        ...
        .then(() => {
          toast('백업 기록을 되돌렸어요')
          if (location.hash === at) void renderManage(root)
        })
        .catch((e) => {
          if (location.hash !== at) return
    ...
    // 눌려 두 번째 흐름이 시작될 수 있다. 게이트 대상 화면이라 wake 재게이트의
    // route(false) 재렌더가 이 경로를 리포트 때보다 자주 밟는다.
    syncBusyButtons()
     ...
     * 이유는 §5다 — 서버가 느리거나 죽어 있으면 목록을 기다리는 동안 화면 전체가
     * 빈 화면으로 남는다. 목록은 이 화면의 부가 기능이고, 데이터 관리 버튼들은
     * 로컬 데이터만으로 이미 완성돼 있다.
     *
    ...
    // 요청 자체가 나가지 않고 이 자리는 영원히 비어 있다. 여기서 확인한 값은 목록을
    // 그릴지 판단하는 데만 쓴다 — 클릭 시점에는 각 흐름이 온라인을 다시 확인한다
    // (그 사이 연결이 끊길 수 있다).
    void (async () => {
      const at = location.hash
    ...
    // 연결된 기기(기기 상한 설계 §3). 스냅샷 목록과 같은 규약 — 렌더를 막지 않고
              ...
              btn.addEventListener('click', () => {
                void confirmDialog({
                  ...
                  removeDevice(d.id)
                    .then((r) => {
                      if (r.ok) {
                        // 같은 해시로 재라우팅해도 안전하다(main.ts — 화면마다 #app을
                        // 통째로 갈아 끼우고 상태는 매번 다시 읽는다). 목록을 다시 읽는다.
                        navigate('#/manage')
                      } else {
                        btn.disabled = false
                        // 서버가 만든 사람 말 사유(자기 자신·이미 해제)를 그대로 보여준다.
                        showError(r.reason)
                      }
                    })
                    .catch((e) => {
    ...
    showError('관리 화면을 열지 못했어요.', e)
    root.replaceChildren(el(`<div><button class="step" id="back">← 리포트</button></div>`))
    root.querySelector('#back')!.addEventListener('click', () => navigate('#/report'))
  }
}
