...
describe('다일 시뮬레이션', () => {
  ...
  it('흔들리는 유형은 닫히는 대신 더 많이 출제된다 — 가중치가 올바른 대응 기제다', () => {
    ...
    //
    // 임계가 0.2인 근거: 12개 시드 실측 분포가 min 0.2594 / median 0.3000 / max 0.3219였다
    // (2026-08-26). 이전 임계 0.3은 정상 분포의 중앙값과 같아서 건강한 코드가 시드의 절반쯤
    // 에서 실패했다 — 시드 555가 우연히 통과하고 있었을 뿐이고, 문장제 인물 풀이 늘어 rand
    // 스트림이 밀리자 그 우연이 걷혔다. 0.2는 균등(0.11)과 정상 하한(0.2594) 사이라
    // 검출력을 유지하면서 시드 운에 흔들리지 않는다.
