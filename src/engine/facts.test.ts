...
describe('factAnswer', () => {
  ...
  // 루프 범위는 **풀(FACT_IDS = 2×1~9×9, 72식)이 아니라 파서의 정의역**이다:
  // FACT_ID_RE가 `/^([1-9])×([1-9])$/`라 1단을 포함한 81조합이 정의역이고, 풀은 그
  // 부분집합이다. 둘이 다르다는 것 자체가 여기서 검사하려는 성질이다 — factId/factAnswer는
  // 문자열 코덱이고, 오늘 풀이 어디까지인지와 독립적으로 서로의 역함수여야 한다.
  // (FACT_IDS를 도는 형태는 정의역이 아니라 풀을 검사하게 되어 범위가 오히려 좁아진다.
  //  풀이 정의역 밖으로 넓어지는 경우를 잡고 싶다면 그건 별개의 단언이다 — HANDOFF 참고.)
  it('factId의 역함수다 — 파서 정의역(1..9 × 1..9, 풀 72식보다 넓다) 전부에서 왕복한다', () => {
    for (let a = 1; a <= 9; a++) {
      for (let b = 1; b <= 9; b++) {
