...
export function syncStatus(input: {
  ...
  if (input.authFailed) {
    tone = 'warn'
    // 2026-08-13부터 복구가 앱 안에서 끝난다 — 이 상태줄 바로 아래에 「다시 연결하기」
    // 버튼이 붙는다(기기 상한 설계 §4, home-parent.ts의 serverStatus().then 안).
    // 그 전까지는 DevTools로 deviceKey를 지워야 했고 문구도 SQL 절차를 가리켰다.
    // **문구가 앱 안의 없는 버튼을 가리키지 않게 할 것** — 옛 문구 「새 키를 발급해
    // 주세요」가 그랬고, 그 직후 문구는 반대로 버튼이 생긴 뒤에도 서버를 가리켜
    // 거짓이 됐다. 버튼을 옮기거나 지우면 이 줄도 함께 본다.
    lines.push('기기 키가 거부됐어요 — 아래에서 다시 연결할 수 있어요')
  }
